...
#NT
theme_set(theme_bw())
r10 <- terra::rast('C:/Users/asus/Desktop/paper1/upscaling/LLdata_for_upscaling/CO2/scenario_SI0.tif')
r10.p <- as.data.frame(r10,xy=TRUE)
mean_value <- mean(r10.p$improvement, na.rm = TRUE)
r10.p$improvement[r10.p$improvement < -100] <- mean_value
r10.p$improvement[r10.p$improvement > 30] <- mean_value
mean_value <- mean(r10.p$improvement, na.rm = TRUE)
...
p10 <- ggplot(data = world) + geom_sf(color = "gray50", fill = "#F7F5F2") +
  ...
  scale_fill_gradientn(colours = c("#2c7bb6", "#abd9e9", "#C9E995", "#fdae61", "#d7191c"), limits = c(-100, 70), na.value = "gray80", name = expression('CO'[2] * ' Change (%)'), guide = "none") +
  theme_void() +
...
hist_plot <- ggplot(r10.p, aes(x = improvement)) +
  ...
  annotate("text", x = mean_value -20, y = 0.06, label = paste0("Mean: ", mean_formatted, "%"), vjust = 2, color = "#E16462", fontface = "bold", size = 1.5) +
...
#RT
theme_set(theme_bw())
r11 <- terra::rast('C:/Users/asus/Desktop/paper1/upscaling/LLdata_for_upscaling/CO2/scenario_SI1.tif')
r11.p <- as.data.frame(r11,xy=TRUE)
mean_value <- mean(r11.p$improvement, na.rm = TRUE)
r11.p$improvement[r11.p$improvement < -70] <- mean_value
r11.p$improvement[r11.p$improvement > 20] <- mean_value
mean_value <- mean(r11.p$improvement, na.rm = TRUE)
mean_formatted <- sprintf("%.2f", mean_value)
world <- ne_countries(scale = "medium", returnclass = "sf")
...
hist_plot <- ggplot(r11.p, aes(x = improvement)) +
  ...
  geom_density(color = "#E16462", linewidth = 0.3) +
  geom_vline(xintercept = mean_value, color = "#FDE725", linewidth = 0.3, linetype = "dashed") +
  annotate("text", x = mean_value -10, y = 0.07, label = paste0("Mean: ", mean_formatted, "%"), vjust = 2, color = "#E16462", fontface = "bold", size = 1.5) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.01)), breaks = seq(0, 0.06, by = 0.03)) +
  labs(x = expression('CO'[2] * ' Change (%)'), y = "Density") +
  theme_classic() +
  theme(panel.background = element_rect(fill = NA, colour = NA),
...
#RI
theme_set(theme_bw())
r12 <- terra::rast('C:/Users/asus/Desktop/paper1/upscaling/LLdata_for_upscaling/CO2/scenario_SI2.tif')
r12.p <- as.data.frame(r12,xy=TRUE)
mean_value <- mean(r12.p$improvement, na.rm = TRUE)
r12.p$improvement[r12.p$improvement > 40] <- mean_value
r12.p$improvement[r12.p$improvement < -40] <- mean_value
mean_value <- mean(r12.p$improvement, na.rm = TRUE)
mean_formatted <- sprintf("%.2f", mean_value)
world <- ne_countries(scale = "medium", returnclass = "sf")
p12 <- ggplot(data = world) + geom_sf(color = "gray50", fill = "#F7F5F2") +
  ...
  scale_fill_gradientn(colours = c("#2c7bb6", "#abd9e9", "#C9E995", "#fdae61", "#d7191c"), limits = c(-100, 70), na.value = "gray80", name = expression('CO'[2] * ' Change (%)'),
    guide = "none"
  ) +
...
hist_plot <- ggplot(r12.p, aes(x = improvement)) +
  ...
  geom_density(color = "#E16462", linewidth = 0.3) +
  geom_vline(xintercept = mean_value, color = "#FDE725", linewidth = 0.3, linetype = "dashed") +
  annotate("text", x = mean_value, y = 0.1, label = paste0("Mean: ", mean_formatted, "%"), vjust = 2, color = "#E16462", fontface = "bold", size = 1.5) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.01)), breaks = seq(0, 0.1, by = 0.05)) +
  labs(x = expression('CO'[2] * ' Change (%)'), y = "Density") +
  theme_classic() +
  theme(panel.background = element_rect(fill = NA, colour = NA),
        plot.background  = element_rect(fill = NA, colour = NA),
...
#ROT
theme_set(theme_bw())
r13 <- terra::rast('C:/Users/asus/Desktop/paper1/upscaling/LLdata_for_upscaling/CO2/scenario_SI3.tif')
...
hist_plot <- ggplot(r13.p, aes(x = improvement)) +
  ...
  geom_density(color = "#E16462", linewidth = 0.3) +
  geom_vline(xintercept = mean_value, color = "#FDE725", linewidth = 0.3, linetype = "dashed") +
  annotate("text", x = mean_value  , y = 0.08, label = paste0("Mean: ", mean_formatted, "%"), vjust = 2, color = "#E16462", fontface = "bold", size = 1.5) +
...
create_colorbar_legend <- function() {
  dummy_plot <- ggplot() +
    ...
    scale_color_gradientn(colours = c("#2c7bb6", "#abd9e9", "#C9E995", "#fdae61", "#d7191c"),
                          limits = c(-100, 70),
                          name = "Relative Change (%)",
                          breaks = seq(-100, 70, by = 50)) +
    theme_void() +
...
ggsave("C:/Users/asus/Desktop/paper1/Supplementary_Fig_10.tiff",
       plot = final_plot,
