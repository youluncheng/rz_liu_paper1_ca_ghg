...
for(i in d02.treat$treatment){
  ...
  out2[[i]] <- data.table(mean = as.numeric((exp(r_ch4$b)-1)*100),
...
r_ch4_0 <- rma.mv(yi,vi, data = d02,random= list(~ 1|study_id), method="REML",sparse = TRUE)
...
for(i in var.sel){
  ...
  out1.est[[i]] <- data.table(var = i,
                              varname = gsub('factor\\(varsel\\)','',rownames(r_ch4_1$b)),
                              mean = round(as.numeric(r_ch4_1$b),3),
                              se = round(as.numeric(r_ch4_1$se),3),
                              ci.lb = round(as.numeric(r_ch4_1$ci.lb),3),
                              ci.ub = round(as.numeric(r_ch4_1$ci.ub),3),
                              pval = round(as.numeric(r_ch4_1$pval),3))
  
  out1.sum[[i]] <- data.table(var = i,
                              AIC = r_ch4_1$fit.stats[4,2],
                              ll = r_ch4_1$fit.stats[1,2],
                              ll_impr = round(100 * (1-r_ch4_1$fit.stats[1,2]/r_ch4_0$fit.stats[1,2]),2),
                              r2_impr = round(100*max(0,(sum(r_ch4_0$sigma2)-sum(r_ch4_1$sigma2))/sum(r_ch4_0$sigma2)),2),
                              pval = round(anova(r_ch4_1,r_ch4_0)$pval,3)
  )
}
...
fwrite(out1.est, "C:/Users/asus/Desktop/paper1/meta-regression/meta_regression_ch4_estimates.csv")
fwrite(out1.sum, "C:/Users/asus/Desktop/paper1/meta-regression/meta_regression_ch4_summary.csv")
...
out = estats(model_new = r_ch4_final,model_base = r_ch4_0)
print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
summary(r_ch4_final)

summary_output <- summary(r_ch4_final)
...
QE <- summary_output$QE %||% r_ch4_final$QE

df_QE <- summary_output$df.QE %||% (r_ch4_final$k - r_ch4_final$p)
...
write.csv(export_table, "C:/Users/asus/Desktop/paper1/meta-regression/r_ch4_try_summary_full.csv", row.names = FALSE)
