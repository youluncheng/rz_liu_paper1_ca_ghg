...
data_pca <- data %>% 
  filter(!is.na(n2ot_mean) & !is.na(n2oc_mean))
...
data_pca$n2o_ratio <- data_pca$n2ot_mean / data_pca$n2oc_mean
...
analyze_management <- function(mgmt, y_limits = NULL) {
  ...
  if(nrow(subset_data) > 10) {
    model_formula <- as.formula(paste("n2o_ratio ~", paste(feature_columns, collapse = " + ")))
    set.seed(123)
    ...
    p <- ggplot() +
      ...
      geom_text(data = importance_factor.scale,
                ...
                hjust = ifelse(importance_factor.scale$`%IncMSE` >= 0, -0.1, 1.8),
                size = 3, 
                nudge_y = 0.3)
    p <- p +
      annotate('text', label = sprintf('n = %d', nrow(subset_data)), x = 1, y = y_limits[2] * 0.81, size = 3, hjust = 0) +
      annotate('text', label = sprintf('R² = %.3f', mean(factor_rfP[["rf"]][["rsq"]])), x = 2, y = y_limits[2] * 0.81, size = 3, hjust = 0)
    plot_list[[mgmt]] <<- p
    ggsave(p, file = paste0("n2o_importance_", mgmt, ".pdf"), width = 6, height = 8)
    return(p)
  }
}
p_RES <- analyze_management("RES", y_limits = c(0, 13))  
p_CC <- analyze_management("CC", y_limits = c(0, 24))    
p_NT <- analyze_management("NT", y_limits = c(0, 17))    
p_RT <- analyze_management("RT", y_limits = c(0, 15))    
p_ROT <- analyze_management("ROT", y_limits = c(0, 17)) 
p_RI <- analyze_management("RI", y_limits = c(0, 17))    
...
ggsave("C:/Users/asus/Desktop/paper1/Supplementary_Fig_8.tiff",
       plot = p_combined,
