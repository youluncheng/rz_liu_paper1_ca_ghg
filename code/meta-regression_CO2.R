...
for(i in d02.treat$treatment){
  ...
  out2[[i]] <- data.table(mean = as.numeric((exp(r_co2$b)-1)*100),
...
r_co2_0 <- rma.mv(yi,vi, data = d02,random= list(~ 1|study_id), method="REML",sparse = TRUE)
...
for(i in var.sel){
  ...
  out1.est[[i]] <- data.table(var = i,
                              varname = gsub('factor\\(varsel\\)','',rownames(r_co2_1$b)),
                              mean = round(as.numeric(r_co2_1$b),3),
                              se = round(as.numeric(r_co2_1$se),3),
                              ci.lb = round(as.numeric(r_co2_1$ci.lb),3),
                              ci.ub = round(as.numeric(r_co2_1$ci.ub),3),
                              pval = round(as.numeric(r_co2_1$pval),3))
  
  out1.sum[[i]] <- data.table(var = i,
                              AIC = r_co2_1$fit.stats[4,2],
                              ll = r_co2_1$fit.stats[1,2],
                              ll_impr = round(100 * (1-r_co2_1$fit.stats[1,2]/r_co2_0$fit.stats[1,2]),2),
                              r2_impr = round(100*max(0,(sum(r_co2_0$sigma2)-sum(r_co2_1$sigma2))/sum(r_co2_0$sigma2)),2),
                              pval = round(anova(r_co2_1,r_co2_0)$pval,3)
  )
}
...
fwrite(out1.est, "C:/Users/asus/Desktop/paper1/meta-regression/meta_regression_co2_estimates.csv")
fwrite(out1.sum, "C:/Users/asus/Desktop/paper1/meta-regression/meta_regression_co2_summary.csv")
...
out = estats(model_new = r_co2_final,model_base = r_co2_0)
print(paste0('model improved the log likelyhood with ',round(out$ll_impr,1),'%'))
summary(r_co2_final)

summary_output <- summary(r_co2_final)
...
QE <- summary_output$QE %||% r_co2_final$QE

df_QE <- summary_output$df.QE %||% (r_co2_final$k - r_co2_final$p)
...
write.csv(export_table, "C:/Users/asus/Desktop/paper1/meta-regression/r_co2_try_summary_full.csv", row.names = FALSE)
