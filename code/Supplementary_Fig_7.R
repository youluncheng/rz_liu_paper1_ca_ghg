...
data_pca <- data %>% 
  filter(!is.na(ch4t_mean) & !is.na(ch4c_mean))
...
data_pca$ch4_ratio <- data_pca$ch4t_mean / data_pca$ch4c_mean
...
analyze_management <- function(mgmt, y_limits = NULL) {
  ...
  if(nrow(subset_data) > 10) {
    model_formula <- as.formula(paste("ch4_ratio ~", paste(feature_columns, collapse = " + ")))
    set.seed(123)
    ...
    p <- ggplot() +
      ...
      geom_text(data = importance_factor.scale,
                ...
                size = 3, 
                nudge_y = 0.3)
    y_stat <- ifelse(mgmt == "NT", y_limits[2] * 0.80, ifelse(mgmt == "RT", y_limits[2] * 0.51, y_limits[2] * 0.81))
    p <- p +
    ...
    plot_list[[mgmt]] <<- p
    ggsave(p, file = paste0("ch4_importance_", mgmt, ".pdf"), width = 6, height = 8)
    return(p)
  }
}
p_RES <- analyze_management("RES", y_limits = c(0, 22))  
p_CC <- analyze_management("CC", y_limits = c(0, 18))    
p_NT <- analyze_management("NT", y_limits = c(0, 13))    
p_RT <- analyze_management("RT", y_limits = c(-6, 4))    
p_ROT <- analyze_management("ROT", y_limits = c(0, 13)) 
p_RI <- analyze_management("RI", y_limits = c(0, 13))    
...
ggsave("C:/Users/asus/Desktop/paper1/Supplementary_Fig_7.tiff",
       plot = p_combined,
