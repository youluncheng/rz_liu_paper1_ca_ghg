...
data_pca <- data %>% 
  filter(!is.na(co2t_mean) & !is.na(co2c_mean))
...
data_pca$co2_ratio <- data_pca$co2t_mean / data_pca$co2c_mean
...
results_list <- list()
plot_list <- list()
label_map <- c(mat = "MAT", map = "MAP", ph = "pH", clay = "Clay", soc = "SOC", n_dose = "N dose", crop_type = "Crop type")
analyze_management <- function(mgmt, y_limits = NULL) {
  ...
  if(nrow(subset_data) > 10) {
    model_formula <- as.formula(paste("co2_ratio ~", paste(feature_columns, collapse = " + ")))
    set.seed(123)
    ...
    p <- ggplot() +
      ...
      geom_text(data = importance_factor.scale,
                ...
                size = 3, 
                nudge_y = 0.3)
    y_stat <- ifelse(mgmt=="NT", 8.3, ifelse(mgmt=="RT", 7.8, y_limits[2] * 0.82))
    p <- p +
    ...
    plot_list[[mgmt]] <<- p
    ggsave(p, file = paste0("co2_importance_", mgmt, ".pdf"), width = 6, height = 8)
    return(p)
  }
}
p_RES <- analyze_management("RES", y_limits = c(0, 25))  
p_CC <- analyze_management("CC", y_limits = c(0, 14))     
p_NT <- analyze_management("NT", y_limits = c(-3, 11))    
p_RT <- analyze_management("RT", y_limits = c(-5, 11))    
p_ROT <- analyze_management("ROT", y_limits = c(0, 18)) 
p_RI <- analyze_management("RI", y_limits = c(0, 18))    
...
ggsave("C:/Users/asus/Desktop/paper1/Supplementary_Fig_6.tiff",
       plot = p_combined,
